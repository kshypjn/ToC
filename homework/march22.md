For a fixed $M$ and $w$, let $Val_{M,w}$ be the set of strings that encode valid accepting computation histories of $M$ on $w$. It is a standard fact that the complement $\overline{Val_{M,w}}$ is context-free: we can build a CFG $G$ such that $L(G) = \overline{Val_{M,w}}$ (e.g. by taking the union of CFLs that each capture one type of “invalid” string—wrong format, wrong initial config, wrong transition, or non-accepting).
